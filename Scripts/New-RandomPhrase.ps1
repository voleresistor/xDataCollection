...
function New-RandomPhrase {
    ...
    $Adjectives = Get-RandomWords -Limit ($Limit*10) -IncludePartOfSpeech adjective -HasDictionaryDef true
    $Nouns = Get-RandomWords -Limit ($Limit*10) -IncludePartOfSpeech noun -HasDictionaryDef true

    if ($WordsPer -ge 3) {
        $Verbs = Get-RandomWords -Limit ($Limit*10) -IncludePartOfSpeech verb -HasDictionaryDef true
    }

    if ($WordsPer -eq 4) {
        $Adverbs = Get-RandomWords -Limit ($Limit*10) -IncludePartOfSpeech adverb -HasDictionaryDef true
    }
    ...
    while ($i -le $Limit) {
        ...
        # Remove current words from the list so they don't appear again this run
...
<#
    ...
    CHANGELOG
    02/25/2020
        Rewrite to meet PowerShell style guide. Increase number of words per query
